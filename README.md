...
这个仓库用于保存源码、文档和后续更新记录。打包产物默认放在本地 `release/`，不提交到 Git；需要分发时建议上传到 GitHub Releases 或直接拷贝 `release/Lan-Control-Hub-0.1.0-win-x64-portable.exe`。
...
- 设备信任：加入密钥、设备公钥签名、控制消息加密、自动信任开关、手动信任和撤销信任。
...
4. 在任意一台电脑的工作台里，确认其他设备显示在线。
5. 如需更严格权限，在设置页关闭“自动信任新设备”，之后新设备需要手动点“信任”。
...
```text
release/Lan-Control-Hub-0.1.0-win-x64-portable.exe
release/Lan-Control-Hub-0.1.0-win-x64-setup.exe
```
