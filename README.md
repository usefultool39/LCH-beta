...
- 手机控制台：已加入房间的电脑会在 `46882/mobile/` 提供移动端 Web/PWA。默认是基础控制面板；预设命令、任意命令、Claude Code / MiniMax-M3 手机智能体和跨设备批量执行需要在桌面端手动开启 Agent Gateway 高级模式。
...
当前推荐使用 `v0.15.1`。这一版是当前阶段 checkpoint：保留 `v0.15.0` 的手机命令控制台能力，并补齐阶段总结、测试清单和 release 文档，适合先停下来完整测试。
...
手机端可以打开任意一台已加入房间电脑的 `http://电脑IP:46882/mobile/`。离开局域网后，手机可以通过 Tailscale 访问一台网关电脑。默认基础模式只提供设备状态、任务查看和网关本机快捷动作；如果确实需要通过网关电脑操作其它已信任设备，先在桌面端“设置 / 系统 / 高级工具”开启 Agent Gateway。开启后，手机端会显示“智能体”和“命令”页：“智能体”页固定连接网关电脑上的 Claude Code，并以 `MiniMax-M3` 模型启动；“命令”页可选择“网关本机 / 选中设备 / 全部在线”，语音识别会先填入命令或匹配快捷动作，执行前仍需确认。详细边界见 [docs/移动端控制台.md](docs/移动端控制台.md)。
...
```text
release/Lan-Control-Hub-0.15.1-win-x64-portable.exe
release/Lan-Control-Hub-0.15.1-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.15.1 release"
git push
```
