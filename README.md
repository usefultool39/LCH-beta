...
当前推荐使用 `v0.12.1`。这是对 v0.12.0 群组会话同步迭代后的最终整理版，功能行为不额外变更，重点是统一版本号、发布说明和使用文档，方便直接作为最新工作版部署。
...
```text
release/Lan-Control-Hub-0.12.1-win-x64-portable.exe
release/Lan-Control-Hub-0.12.1-win-x64-setup.exe
```
...
git status
git add .
git commit -m "prepare v0.12.1 release"
git push
```
