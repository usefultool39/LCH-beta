...
- 手机控制台：已加入房间的电脑会在 `46882/mobile/` 提供移动端 Web/PWA，手机可在局域网或 Tailscale 中通过白名单快捷动作控制设备。
...
当前推荐使用 `v0.14.0`。这一版在 `v0.13.0` 的局域网房间大厅基础上，新增手机 Web/PWA 控制台：同局域网或 Tailscale 下可以打开 `http://电脑IP:46882/mobile/`，用房间密钥登录后执行白名单快捷动作。
...
手机端可以打开任意一台已加入房间电脑的 `http://电脑IP:46882/mobile/`。离开局域网后，手机可以通过 Tailscale 访问一台网关电脑，再由这台网关电脑通过 Lan Control Hub 的局域网控制通道操作其它已信任设备；这不是网络层自动桥接，而是应用层中转。详细边界见 [docs/移动端控制台.md](docs/移动端控制台.md)。
...
```text
release/Lan-Control-Hub-0.14.0-win-x64-portable.exe
release/Lan-Control-Hub-0.14.0-win-x64-setup.exe
```
