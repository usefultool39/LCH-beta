...
- 多设备工作台：查看在线设备、稳定设备代号、系统名、别名、房间、收藏、只读状态，以及当前连接走的是局域网、Tailscale 还是手动入口。
- 房间大厅：房间代表同一套加入密钥和信任关系；新设备可扫描同一局域网里的入口，跨网时再添加 Tailscale 100.x 地址。
...
1. 第一台电脑打开 App，创建一个 LCH 房间。
2. 进入设置页，复制这个房间的“房间密码 / 加入密钥”。
3. 其他电脑打开 App，同一局域网可扫描附近入口并选中要加入的房间；跨网时直接粘贴同一房间密钥。
4. 粘贴房间密码加入。
...
当前推荐使用 `v0.19.0`。这一版包含 v0.18.0 的所有功能（Tailscale 子网扫描、隐身房间、信任向导、路由排序/延迟、`lch` CLI App Paths 开关），并把 Phase D 的「按延迟自动选路」改为默认开启。
...
跨外网访问推荐使用 Tailscale、ZeroTier 或 WireGuard 这类虚拟专网。加入同一个虚拟网络后，在设置页的“网络 / 跨网入口 / Tailscale”里输入对方虚拟 IP，例如 `100.x.x.x`；如果对方 Web 端口不是默认值，可以填 `100.x.x.x:46882`。连接成功后设备会进入待信任列表，仍然需要使用同一个加入密钥并完成双方信任。

同一台设备可能同时有局域网 192.x 地址和 Tailscale 100.x 地址。工作台会把这些入口分开展示为“局域网入口”“Tailscale入口”“手动入口”，并标出当前正在使用的入口；设备名、别名、设备代号和 IP 不再混在同一行里。
