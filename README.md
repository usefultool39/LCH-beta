...
- 聊天增强：支持本地搜索、回复引用、Markdown/代码块渲染和基础 reaction，旧设备会降级为普通文本。
...
```text
Lan-Control-Hub-0.8.0-win-x64-portable.exe
Lan-Control-Hub-0.8.0-win-x64-setup.exe
Lan-Control-Hub-0.8.0-mac-x64.zip
Lan-Control-Hub-0.8.0-mac-arm64.zip
SHA256SUMS.txt
```
...
每台设备的聊天/文件会话默认保留最近 1000 条事件，超过后自动裁剪旧记录。聊天支持本地搜索、回复引用、基础 reaction，以及 `**加粗**`、`` `代码` `` 和 fenced code block 这类安全的 Markdown 子集。收到大量消息可以正常接收，但当前不是企业 IM，不做云端同步和无限历史归档。
